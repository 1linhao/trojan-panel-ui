...
const LiquidForm = {
  ...
  provide() {
    return { liquidForm: this };
  },
  watch: { rules: { deep: true, handler(value) {
    this.controller.setRules(value);
  } } },
  beforeDestroy() {
  ...
  methods: {
    async validate(fields, callback) {
      ...
      callback == null ? void 0 : callback(result.valid, result);
      return result;
    },
    validateField(field) {
      return this.controller.validateField(field);
    },
    clearValidate(fields) {
      return this.controller.clear(fields);
    },
    async submit(event) {
      var _a;
